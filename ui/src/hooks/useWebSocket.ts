...
export function useWebSocket() {
    ...
    function connect() {
      ...
      ws.onmessage = (e) => {
        try {
          ...
          if (msg.type === 'ai-start' || msg.type === 'ai-stream') setAiWorking(true)
          if (msg.type === 'ai-done') {
            setAiWorking(false)
            setLastResult({ text: msg.text || '', channelId: msg.channelId || 'default' })
          }
          if (msg.type === 'ai-error') setAiWorking(false)
  ...
  return { connected, aiWorking, lastResult, clearLastResult, addListener }
}
