...
export function createMarketDataTools(store: MarketDataStore) {
  return {
    marketData: tool({
      description: 'Query market data from the database: candles, latest price, status, or summary.',
      inputSchema: z.object({
        action: z.enum(['candles', 'price', 'status', 'summary']).describe(
          'candles: historical OHLCV data | price: latest price | status: connection status | summary: statistical summary',
        ),
        exchange: z.string().optional().describe('Exchange name, e.g. "binance" or "bybit"'),
        symbol: z.string().optional().describe('Trading pair, e.g. "BTC/USDT"'),
        timeframe: z.string().optional().default('1h').describe('Candle timeframe, e.g. "1m", "5m", "1h", "4h", "1d"'),
        ...
        period: z.string().optional().default('24h').describe('Lookback period for summary, e.g. "24h", "7d", "30d"'),
      }),
      execute: async (input) => {
        switch (input.action) {
          case 'candles': {
            if (!input.exchange || !input.symbol) return { error: 'exchange and symbol are required for candles' }
            ...
            const candles = store.getCandles(input.exchange, input.symbol, timeframe, sinceMs, untilMs, limit)
            if (!candles.length) return { error: `No candle data found for ${input.exchange}/${input.symbol}/${timeframe}` }
            const range = store.getDateRange(input.exchange, input.symbol, timeframe)
            const totalAvailable = store.getCandleCount(input.exchange, input.symbol, timeframe)
            const closes = candles.map(c => c.close)
            const highs = candles.map(c => c.high)
            const lows = candles.map(c => c.low)
            const vols = candles.map(c => c.volume)
            return {
              exchange: input.exchange, symbol: input.symbol, timeframe,
          ...
          case 'price': {
            if (!input.exchange || !input.symbol) return { error: 'exchange and symbol are required for price' }
          ...
          case 'summary': {
            if (!input.exchange || !input.symbol) return { error: 'exchange and symbol are required for summary' }
            const timeframe = input.timeframe || '1h'
            const periodMs = parsePeriod(input.period || '24h')
            const since = Date.now() - periodMs
            const candles = store.getCandles(input.exchange, input.symbol, timeframe, since, undefined, 10000)
            if (!candles.length) return { error: `No data for ${input.exchange}/${input.symbol}/${timeframe}` }
